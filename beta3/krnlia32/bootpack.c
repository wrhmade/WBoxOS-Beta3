...
void HariMain(void)
{
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	for (;;) {
		update_date_time(sht_back);
		
		//putfonts8_asc(binfo->vram, binfo->scrnx, 615, 523, COL8_000000, "WBoxOS Operating System");
		putfonts8_asc_sht(sht_back, binfo->scrnx-185, binfo->scrny-77, 0, BACKC,"WBoxOS Operating System",24);
		//putfonts8_asc(binfo->vram, binfo->scrnx, 655, 539, COL8_000000, "Version 0.1 [Beta]");
		putfonts8_asc_sht(sht_back, binfo->scrnx-145, binfo->scrny-61, 0, BACKC,"Version 0.5 [Beta]",18);

		//putfonts8_asc(binfo->vram, binfo->scrnx, 385, 555, COL8_000000, "WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.");
		putfonts8_asc_sht(sht_back, binfo->scrnx-415, binfo->scrny-45, 0, BACKC,"WARNING:THIS IS A BETA VERSION THAT MAY BE UNSTABLE.",52);
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* キーボードデータ */
				...
				if (i == 256 + 0x3c && key_shift != 0) {	/* Shift+F2 */
					...
					key_win = open_console(shtctl, memtotal,"ｿﾘﾖﾆﾌｨ");
					sheet_slide(key_win, 32, 4);
				...
				if (mouse_decode(&mdec, i - 512) != 0) {
					...
					if ((mdec.btn & 0x01) != 0) {
						...
						if((mx >= 20 && mx <=52) && (my >= 20 && my <=52))
						{
							...
							x = mx - sht->vx0;
							y = my - sht->vy0;
							if (!(0 <= x && x < sht->bxsize && 0 <= y && y < sht->bysize) || shtctl->top - 1==0) 
							{
